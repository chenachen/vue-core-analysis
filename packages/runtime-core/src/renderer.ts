...
function baseCreateRenderer(
  ...
  const patchElement = (
    ...
    if (patchFlag > 0) {
      ...
      // patchFlag 说明当前节点来自编译器生成代码，可以按标记位直接命中更快的更新分支。
      // 这些快速路径建立在新旧 vnode 结构稳定的前提上：它们位于模板中的同一位置。
      if (patchFlag & PatchFlags.FULL_PROPS) {
        // 存在动态 key，无法只看少数几个属性，必须退回完整 props diff
        // element props contain dynamic keys, full diff needed
        patchProps(el, oldProps, newProps, parentComponent, namespace)
      } else {
        // class
        // this flag is matched when the element has dynamic class bindings.
        // 命中该标记说明 class 可能发生变化，只需定点检查并更新 class
        ...
        // 命中该标记说明 style 可能发生变化，只需定点更新 style
      ...
      // 命中该标记说明该元素只有一个动态文本子节点，可以直接更新 textContent
   ...
   * 对元素执行完整的 props diff。
   *
   * 这里分成三步：先删旧 props，再更新普通新 props，最后单独处理 `value`。
   * `value` 被延后是因为它在某些表单元素上有顺序要求，并且需要强制写入。
   */
  const patchProps = (
    el: RendererElement,
    oldProps: Data,
   ...
   * 处理组件 vnode。
   *
   * 组件与元素一样也分挂载和更新两条分支，但额外要处理 KeepAlive 的激活逻辑。
   * runtime-core 在这里接管组件生命周期，并把最终渲染出的子树继续交回 `patch`。
   */
   ...
   * 为组件建立渲染副作用。
   *
   * 这是运行时渲染闭环的关键一环：首次执行时完成 mount，后续依赖变更时由调度器
   * 重新触发同一个 effect，得到新的子树并继续递归 patch。
   */
